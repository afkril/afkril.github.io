...
const AsociacionesModule = (() => {
    ...
    function getRef(tipo) {
        ...
        switch(tipo) {
            ...
            case 'papelera':      return `Papelera_${id}`;
    ...
    async function mostrarSelectorAsociaciones() {
        // En páginas de operador fijo, jamás se muestra el listado
        // completo de asociaciones: en vez de eso, se reintenta cargar
        // únicamente el perfil fijo de esta página.
        const idFijo = _idOperadorFijo();
        if (idFijo) {
            try { await _cargarPerfilFijo(idFijo); }
            catch(e) { console.error('[Asociaciones] No se pudo recargar el perfil fijo:', e); }
            return;
        }
    ...
    function _activarYCerrarSelector(id, datos) {
        ...
        // index.html ya no tiene formulario propio (vive aparte en las
        // páginas de operador fijo, ej. Jer.html): al elegir el operador
        // se entra directo a su panel administrativo. El usuario ya pasó
        // por el login, así que se marca la sesión admin de este perfil.
    ...
    function cambiarAsociacion() {
        // En páginas de operador fijo no existe "cambiar de asociación"
        if (_idOperadorFijo()) return;
        ...
        if (typeof datosNutricionales !== 'undefined') {
            datosNutricionales = [];
            datosNutricionalesFiltrados = [];
            if (typeof paginaNutricional !== 'undefined') paginaNutricional = 1;
        }

        mostrarSelectorAsociaciones();
    }

    // ── Operador fijo (páginas dedicadas por operador) ────────
    // Si el HTML define window.OPERADOR_FIJO = 'jer' (por ejemplo),
    // esta página queda "encadenada" a esa asociación: no se muestra
    // el selector general y se oculta el botón "Cambiar Asociación".
    function _idOperadorFijo() {
        return (typeof window.OPERADOR_FIJO === 'string' && window.OPERADOR_FIJO.trim())
            ? window.OPERADOR_FIJO.trim()
            : null;
    }

    function _aplicarModoOperadorFijo() {
        if (!_idOperadorFijo()) return;
        const btn = document.getElementById('btnCambiarAsociacion');
        if (btn) btn.remove();
    }

    async function _cargarPerfilFijo(id) {
        const snap = await _getDB().ref(`${PATHS.asociaciones}/${id}`).once('value');
        const datos = snap.val();
        if (!datos) {
            document.body.innerHTML = `<div style="display:flex;align-items:center;justify-content:center;
                height:100vh;font-family:sans-serif;text-align:center;padding:20px;color:#334155;">
                <div>
                    <div style="font-size:3rem">❌</div>
                    <h2 style="margin:12px 0 4px">Operador no encontrado</h2>
                    <p style="color:#64748b">El ID "${id}" no existe o fue eliminado. Contacta al administrador.</p>
                </div>
            </div>`;
            throw new Error(`Asociación "${id}" no encontrada`);
        }
        activarPerfil(id, datos);
    }
    ...
    async function init() {
        ...
        // ── Página dedicada de un operador (Jer.html, T3.html, etc.) ──
        if (idFijo) {
            _aplicarModoOperadorFijo();
            // Reutiliza la sesión si ya es la misma asociación fija;
            // si la sesión guardada es de OTRA asociación, la reemplaza
            // por la fija (evita que quede "pegado" el perfil anterior).
            const sesionId = sessionStorage.getItem('asoc_id');
            if (sesionId === idFijo && recuperarPerfilDeSesion()) return;
            await _cargarPerfilFijo(idFijo);
            return;
        }

        // Si ya hay perfil en sesión, usarlo (típico al recargar la página
        // tras haber elegido operador). index.html ya no tiene formulario
        // que mostrar por defecto, así que hay que abrir el panel admin.
