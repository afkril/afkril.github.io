		...
		function cargarRegistrosAdmin() {
			...
			const filtrados = registros.filter(reg => {
				...
				if (filtroBuscar) {
					const opNombreBuscar = reg.detalles?.operador ? (window.operadoresNombres?.[reg.detalles.operador] || reg.detalles.operador) : '';
					const buscarEn = [
						reg.ip,
						reg.deviceId,
			...
			if (pagina.length === 0) {
				...
				pagina.forEach((reg, idx) => {
					...
					if (reg.detalles) {
						...
						const opNombre = reg.detalles.operadorNombre || (reg.detalles.operador ? (window.operadoresNombres?.[reg.detalles.operador] || reg.detalles.operador) : null);
						if (opNombre) detallesTexto += `• 👷${opNombre}`;
					}

					const accionLabels = {
						GENERAR_LISTA: '📋 Semanal',
						GENERAR_LISTA_MENSUAL: '📅 Mensual',
						CREAR_OPERADOR: '🆕 Operador creado',
						ELIMINAR_OPERADOR: '🗑️ Operador eliminado',
						CREAR_DIRECTORIO: '📂 Directorio creado',
						ACTUALIZAR_DIRECTORIO: '📂 Directorio actualizado',
						ELIMINAR_DIRECTORIO: '🗑️ Directorio eliminado'
					};
						...
						<tr style="border-bottom: 1px solid var(--border); background: ${esPar ? 'transparent' : 'rgba(255,255,255,0.02)'}; transition: background 0.15s;" 
							...
							<td style="padding: 0.5rem 0.75rem;">
								<span style="background: rgba(99,102,241,0.12); color: #818cf8; padding: 0.2rem 0.5rem; border-radius: 0.25rem; font-size: 0.7rem; white-space: nowrap;">
									${accionLabels[reg.accion] || reg.accion || 'N/A'}
								</span>
							</td>
							<td style="padding: 0.5rem 0.75rem;">
							...
							<td style="padding: 0.5rem 0.75rem; text-align: center; white-space: nowrap;">
								<button onclick="verDetalleRegistro(${regIdx})" title="Ver detalle" style="background: rgba(99,102,241,0.15); border: 1px solid rgba(99,102,241,0.3); color: #818cf8; border-radius: 0.375rem; cursor: pointer; padding: 0.2rem 0.5rem; font-size: 0.7rem; transition: all 0.2s; margin-right: 0.25rem;" 
									onmouseover="this.style.background='rgba(99,102,241,0.3)'" 
									onmouseout="this.style.background='rgba(99,102,241,0.15)'">
									🔍
								</button>
								<button onclick="eliminarRegistroAuditoria(${regIdx})" title="Eliminar este registro" style="background: rgba(239,68,68,0.12); border: 1px solid rgba(239,68,68,0.3); color: #f87171; border-radius: 0.375rem; cursor: pointer; padding: 0.2rem 0.5rem; font-size: 0.7rem; transition: all 0.2s;" 
		...
		function verDetalleRegistro(idx) {
			...
			content.innerHTML = `
				...
				<div style="margin-bottom: 0.875rem;">
					...
					${seccion('op', 'Operador', reg.detalles?.operadorNombre || (reg.detalles?.operador ? (window.operadoresNombres?.[reg.detalles.operador] || reg.detalles.operador) : null))}
				</div>
		...
		async function exportarRegistros() {
			...
			const data = registrosAuditoria.map(reg => ({
				...
				'Operador': reg.detalles?.operadorNombre || (reg.detalles?.operador ? (window.operadoresNombres?.[reg.detalles.operador] || reg.detalles.operador) : ''),
				'URL': reg.url || ''
			}));
		...
		async function limpiarRegistros() {
			try {
				await mostrarConfirm('Se eliminarán TODOS los registros de auditoría de Firebase. Esta acción es irreversible.', {
					titulo: '⚠️ Eliminar registros de auditoría', icono: '🔥', btnOk: 'Sí, eliminar todo', palabraConfirmacion: 'CONFIRMAR'
				});
			} catch { return; }
		...
		// Eliminar UN SOLO registro de auditoría (por índice dentro de registrosAuditoria)
		async function eliminarRegistroAuditoria(idx) {
			const reg = registrosAuditoria[idx];
			if (!reg) {
				showToast('El registro ya no existe (puede que la lista se haya actualizado)', 'error');
				return;
			}

			const fechaTxt = reg.fechaLocal || new Date(reg.fecha).toLocaleString('es-CO');

			try {
				await mostrarConfirm(
					`Se eliminará el registro #${reg.id} (${fechaTxt}). Esta acción no se puede deshacer.`,
					{ titulo: '¿Eliminar este registro?', icono: '🗑️', btnOk: 'Sí, eliminar' }
				);
			} catch { return; }
			...
			try {
				...
				// Quitar también del arreglo local para refrescar la tabla al instante
				registrosAuditoria = registrosAuditoria.filter(r => r.id !== reg.id);

				cargarRegistrosAdmin();
				showToast(`Registro #${reg.id} eliminado`, 'success');
			} catch (error) {
