...
function _getFilteredCurrentExportRows() {
            ...
            const exportData = filtered.map(n => {
                ...
                return {
                    'Estado CUENTAME': _estadoLabel(n.cuentameStatus),
...
function _estadoLabel(status) {
            switch (status) {
                ...
                case 'otro_operador': return 'OTRO OPERADOR';
                default: return 'PENDIENTE';
            }
        }

function exportToExcelCurrent() {
            const exportData = _getFilteredCurrentExportRows();
            ...
            showToast(`Exportados ${exportData.length} registros de la vista actual`, "success");
        }

function exportToCSVCurrent() {
            const exportData = _getFilteredCurrentExportRows();
            if (!exportData) return;
            ...
            a.href = url;
            a.download = `Reporte_Filtrado_JER_${new Date().toISOString().split('T')[0]}.csv`;
            document.body.appendChild(a);
            a.click();
            a.remove();
            URL.revokeObjectURL(url);

            showToast(`Exportados ${exportData.length} registros de la vista actual (CSV)`, "success");
        }
